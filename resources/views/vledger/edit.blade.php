    ...
    <div class="form-group">
        ...
        <select name="visa_country_id" class="form-control" >
            ...
            @foreach($countries as $countryId => $countryName)
                <option value="{{ $countryId }}" {{ $vendor->visa_country_id == $countryId ? 'selected' : '' }}>{{ $countryName }}</option>
            @endforeach
...
<div class="form-group col-md-6">
        ...
        <select name="visa_type" class="form-control" required>
            <option value="{{$vendor->visa_type}}" selected disabled>{{ $vendor->visa_type == "SV" ?
            __('Student Visa') : ($vendor->visa_type == "TV" ? __('Tourist Visa') : ($vendor->visa_type == "WV" ? __('Work Permit Visa') : ($vendor->visa_type == "BV" ? __('Business Visa') : ($vendor->visa_type == "OV" ? __('Others') : __('Select Visa Type')))))
             }}</option>
